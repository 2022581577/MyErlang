...
do_cast({add_log, Operation, TableName, Value}, #state{insert_dict = InsertDict, replace_dict = ReplaceDict} = State) ->
    Value1 = edb_util:make_value_sql(Value),
    case Operation of
        ?OPERATION_INSERT ->
            NewInsertDict = do_add_log(Operation, TableName, "(0, " ++ Value1 ++ ")", InsertDict),
            NewState = State#state{insert_dict = NewInsertDict};
        _ ->
            NewReplaceDict = do_add_log(Operation, TableName, "(" ++ Value1 ++ ")", ReplaceDict),
...
do_add_log(Operation, TableName, Value, Dict) ->
