...
fun_init() ->
    ...
    %% 映射信息先不处理
    %"\tets:new(?ETS_ACCOUNT_INFO,[{keypos,#account_info.account_name},named_table,public,set,{read_concurrency,true}]),     %% 账号角色ID映射\n"
    %"\tets:new(?ETS_USER_ID,[{keypos,#uid_info.user_id},named_table,public,set,{read_concurrency,true}]),                   %% 角色名称映射\n"
    %"\tets:new(?ETS_USER_NAME,[{keypos,#name_info.name},named_table,public,set,{read_concurrency,true}]),                   %% 角色名称映射\n"
    "\tok.\n"
    %"get_account_info(AccName) ->\n"
    %"\tAccountName = util:to_binary(AccName),\n"
    %"\tcase ets:lookup(?ETS_ACCOUNT_INFO,AccountName) of\n"
    %"\t\t[#account_info{} = AccountInfo] ->\n"
    %"\t\t\tAccountInfo;\n"
    %"\t\t[] ->\n"
    %"\t\t\tfalse\n"
    %"\tend.\n\n"
    %"add_account_mapping(AccountName,UserID) ->\n"
    %"\tNewAccountInfo = \n"
    %"\t\tcase get_account_info(AccountName) of\n"
    %"\t\t\t#account_info{user_ids = UserIDs} = AccountInfo ->\n"
    %"\t\t\t\tAccountInfo#account_info{user_ids = [UserID | lists:delete(UserID,UserIDs)]};\n"
    %"\t\t\tfalse ->\n"
    %"\t\t\t\t#account_info{account_name = AccountName,user_ids = [UserID]}\n"
    %"\t\tend,\n"
    %"\t%% 帐号名称映射\n"
    %"\tets:insert(?ETS_ACCOUNT_INFO,NewAccountInfo).\n\n"
