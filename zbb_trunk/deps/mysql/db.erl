...
-export([init/1,test/0,check_log_db/0,stop/0,reconnect/0,switch_db/0]).
...
reconnect() ->
    ...
    ?SYSLOG("switch to db:~p",[DbName]),
    start_mysql_pool(SupPid, ?MYSQL_CONNECT_COUNT),
    ok.

switch_db() ->
    ?SYSLOG("Begin Stop Mysql DB"),
    stop(),
    ?SYSLOG("Stop Mysql Success,Restart It"),
    reconnect(),
    case db_util:check_db_state() of
        false ->
            ?SYSLOG("Error Switch DB Fail,Config:~p,~w,~p",[?CONFIG(db_host),?CONFIG(db_port),?CONFIG(db_name)]),
            false;
        _ ->
            ?SYSLOG("Switch DB Success,Config:~p,~w,~p",[?CONFIG(db_host),?CONFIG(db_port),?CONFIG(db_name)]),
            true
    end.
...
start_mysql(SupPid)->
    ...
    %% DbName = get_db_name(),   %% 连接默认数据库
	DbName = "test",   %% 连接默认数据库
	Port = ?CONFIG(db_port),
	%% mysql:start_link(PoolId, Host, Port, User, Password, Database, LogFun, Encoding)
	Child = {mysql, {mysql, start_link,
					[?MYSQL_POOL,Host,Port,User,Password,DbName,fun(_,_,_,_)-> ok end,Encode]},
    ...
    case supervisor:start_child(SupPid,Child) of
    	{ok,MysqlPid} ->
            ...
            ?SYSLOG("Commect Mysql Success,IP:~ts,Port:~w,MysqlPid:~w",[Host,Port,MysqlPid]),
        	MysqlPid;
    	Other ->
            ?SYSLOG("Commect Mysql Fail,IP:~ts,Port:~w,Error:~ts",[Host,Port,Other]),
...
start_mysql_pool(SupPid,ConnectCount)->
	...
	ConnPidList = [begin
					...
					{ok,ConnectPid} ->
						ConnectPid;
					Other2 ->
                        ?SYSLOG("Commect Mysql Pool Fail,IP:~s,Port:~w,Error:~s",[Host,Port,Other2]),
...
check_log_db() ->
    ...
    timer:sleep(1000),
    %% 确保当前数据库正确
    %% [Database] = db_util:execute("select database()"),            
    %% 检测是否有global_data表
...
%    lists:concat(["log_",Platform,"_",ServerID]).
