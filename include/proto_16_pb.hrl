...
-ifndef(S2C16001_PB_H).
-define(S2C16001_PB_H, true).
-record(s2c16001, {
    list = []
}).
-endif.

-ifndef(C2S16002_PB_H).
-define(C2S16002_PB_H, true).
-record(c2s16002, {
    task_id = erlang:error({required, task_id})
}).
-endif.

-ifndef(S2C16002_PB_H).
-define(S2C16002_PB_H, true).
-record(s2c16002, {
    task_id = erlang:error({required, task_id}),
...
-ifndef(C2S16003_PB_H).
-define(C2S16003_PB_H, true).
-record(c2s16003, {
    task_id = erlang:error({required, task_id})
}).
-endif.

-ifndef(S2C16003_PB_H).
-define(S2C16003_PB_H, true).
-record(s2c16003, {
    task_id = erlang:error({required, task_id}),
    result = erlang:error({required, result}),
    list = []
...
-ifndef(C2S16004_PB_H).
-define(C2S16004_PB_H, true).
-record(c2s16004, {
    task_id = erlang:error({required, task_id})
}).
-endif.

-ifndef(S2C16004_PB_H).
-define(S2C16004_PB_H, true).
-record(s2c16004, {
    task_id = erlang:error({required, task_id}),
    result = erlang:error({required, result})
}).
...
-ifndef(S2C16005_PB_H).
-define(S2C16005_PB_H, true).
-record(s2c16005, {
    list = []
}).
-endif.

-ifndef(P_LOOP_TASK_PB_H).
-define(P_LOOP_TASK_PB_H, true).
-record(p_loop_task, {
    type = erlang:error({required, type}),
    turn = erlang:error({required, turn}),
    list = [],
    award_flag = erlang:error({required, award_flag})
}).
-endif.

-ifndef(P_LOOP_TASK_ITEM_PB_H).
-define(P_LOOP_TASK_ITEM_PB_H, true).
-record(p_loop_task_item, {
    task_id = erlang:error({required, task_id}),
    difficulty = erlang:error({required, difficulty}),
    award = erlang:error({required, award}),
    target_type = erlang:error({required, target_type}),
    target_id = erlang:error({required, target_id}),
    target_count = erlang:error({required, target_count}),
    current_count = erlang:error({required, current_count}),
    task_state = erlang:error({required, task_state}),
    seq = erlang:error({required, seq}),
    user_level = erlang:error({required, user_level})
}).
-endif.

-ifndef(C2S16006_PB_H).
-define(C2S16006_PB_H, true).
-record(c2s16006, {
    type = erlang:error({required, type}),
    seq = erlang:error({required, seq}),
    finish_type = erlang:error({required, finish_type})
}).
-endif.

-ifndef(S2C16006_PB_H).
-define(S2C16006_PB_H, true).
-record(s2c16006, {
    result = erlang:error({required, result}),
    type = erlang:error({required, type}),
    seq = erlang:error({required, seq})
...
-ifndef(C2S16007_PB_H).
-define(C2S16007_PB_H, true).
-record(c2s16007, {
    type = erlang:error({required, type})
}).
-endif.

-ifndef(C2S16008_PB_H).
-define(C2S16008_PB_H, true).
-record(c2s16008, {
    
}).
-endif.

-ifndef(C2S16009_PB_H).
-define(C2S16009_PB_H, true).
-record(c2s16009, {
    
}).
-endif.

-ifndef(C2S16010_PB_H).
-define(C2S16010_PB_H, true).
-record(c2s16010, {
    seq = erlang:error({required, seq})
}).
-endif.

-ifndef(C2S16011_PB_H).
-define(C2S16011_PB_H, true).
-record(c2s16011, {
    target_type = erlang:error({required, target_type}),
    target_id = erlang:error({required, target_id}),
    target_count = erlang:error({required, target_count})
...
-ifndef(C2S16012_PB_H).
-define(C2S16012_PB_H, true).
-record(c2s16012, {
    type = 3
}).
-endif.
